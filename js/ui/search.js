...
export function initSearch(domRefs, { onPerformSearch } = {}){
  ...
  try{
    ...
    if (sortSel){
      sortSel.value = searchState.sort || 'default';
      sortSel.addEventListener('change', () => {
        searchState.sort = sortSel.value || 'default';
        performSearch();
      });
    }
    if (minScoreInp){
      const applyMin = () => {
        const v = Number(minScoreInp.value);
        searchState.minScore = Number.isFinite(v) && v > 0 ? Math.floor(v) : 0;
        // Do not auto-trigger search on every keypress; wait for change/enter
      };
      minScoreInp.addEventListener('input', applyMin);
      minScoreInp.addEventListener('change', () => { applyMin(); performSearch(); });
    }
  }catch{}
